...
namespace Fosol.Common.Web.Mvc.Helpers
{
    /// <summary>
    /// RouteValuesHelper static class provides helper methods when using a RouteValueDictionary object.
    /// </summary>
    public static class RouteValuesHelper
    {
        ...
        public static RouteValueDictionary MergeRouteValues(string actionName, string controllerName, RouteValueDictionary routeValues)
        {
            ...
            if (routeValues != null)
            {
                foreach (var current in Fosol.Common.Initialization.Assert.IsNotNull(routeValues, new RouteValueDictionary()))
                {
                    routes[current.Key] = current.Value;
                }
            }
