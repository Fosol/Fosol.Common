...
namespace Fosol.Common.ServiceModel.Initialization
{
    public static class Assert
    {
        ...
        public static T Convert<T>(object value, string paramName, string message = null)
        {
            ...
            {
                throw new Faults.ParameterNullFault(paramName, string.Format(message ?? Resources.Strings.Initialization_Convert_ArgumentNullException, paramName)).Raise<Faults.ParameterNullFault>();
            }
            catch (InvalidCastException)
            {
                throw new Faults.ParameterInvalidCastFault(paramName, string.Format(message ?? Resources.Strings.Iniitialization_Convert_InvalidCastException, paramName, typeof(T).Name)).Raise<Faults.ParameterInvalidCastFault>();
            }
            catch (OverflowException)
            {
                throw new Faults.ParameterOverflowFault(paramName, string.Format(message ?? Resources.Strings.Initialization_Convert_OverflowException, paramName, typeof(T).Name)).Raise<Faults.ParameterOverflowFault>();
            }
            catch (FormatException)
            {
                throw new Faults.ParameterFormatFault(paramName, string.Format(message ?? Resources.Strings.Initialization_Convert_FormatException, paramName, typeof(T).Name)).Raise<Faults.ParameterFormatFault>();
            }
        }
        #endregion
    }
